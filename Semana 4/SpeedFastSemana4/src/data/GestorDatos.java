...
public class GestorDatos {
    ...
    public ArrayList<Pedido> leerArchivo() {
        ...
        try {
            ...
            System.out.println("No hay permisos para leer el archivo de datos: " + e.getMessage());
        }

        // Aunque la lectura falle se devuelve la colección, vacía o parcial, para
        // que la aplicación pueda seguir trabajando en memoria.
        return pedidos;
    }

    /**
     * Convierte una línea del archivo en un pedido concreto. Un registro que no
     * se pueda interpretar se descarta con un aviso, de manera que un dato
     * dañado nunca interrumpa la carga del resto de los pedidos.
     * @param linea registro delimitado que será interpretado
     */
    private void cargarPedido(String linea) {
        // Las líneas vacías del final del archivo no son un error, se omiten.
        if (linea.isBlank()) {
            return;
        }
        ...
        try {
            ...
            // Un tipo desconocido deja el pedido en null: se descarta el registro
            // en lugar de dejar un hueco que después haría fallar los recorridos.
            if (isNull(pedido)) {
                System.out.println("Registro ignorado por tener un tipo de pedido desconocido (" + tipo + "): " + linea);
                return;
            }

            pedidos.add(pedido);
        } catch (NumberFormatException e) {
            System.out.println("Registro ignorado porque la distancia o la cantidad no son números válidos: " + linea);
        }
    }
   ...
   private void asegurarArchivo(File archivo) throws IOException {
    ...
    /**
     * Muestra únicamente los pedidos que tienen estado Cancelado.
     */
    public void mostrarCancelados() {
        System.out.println("==== Pedidos cancelados ====");
        boolean hayCancelados = false;
        for (Pedido pedido : pedidos) {
            if (pedido.getEstadoPedido().equalsIgnoreCase("Cancelado")) {
                System.out.println("- " + pedido.getTipoPedido() + " " + pedido.getIdPedido());
                hayCancelados = true;
            }
        }
        if (!hayCancelados) {
            System.out.println("Aún no hay pedidos cancelados.");
        }
    }
}
